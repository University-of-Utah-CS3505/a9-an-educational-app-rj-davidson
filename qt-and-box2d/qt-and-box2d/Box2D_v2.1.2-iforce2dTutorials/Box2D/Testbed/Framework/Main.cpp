...
namespace
{
	int32 testCategoryIndex = 0;
	int32 testCategorySelection = 0;
	int32 testCategoryCount = 0;
	TestEntryCategory* entryCategory;
	int32 testIndex = 0;
...
void SimulationLoop()
{
	...
	if (testCategorySelection != testCategoryIndex)
	{
		for (int id = 0; id < testCount; id++)
			testList->delete_item( id );

		testCategoryIndex = testCategorySelection;
		testSelection = 0;
		entryCategory = g_testEntryCategories + testCategoryIndex;
		testCount = 0;		
		while (entryCategory->entries[testCount].createFcn != NULL)
		{
			testList->add_item(testCount, entryCategory->entries[testCount].name);
			++testCount;
		}
	...
	if (testSelection != testIndex || reloadTest)
	{
		testIndex = testSelection;
		delete test;
		entryCategory = g_testEntryCategories + testCategoryIndex;
		entry = entryCategory->entries + testIndex;
...
{
	testCategoryCount = 0;
	while (g_testEntryCategories[testCategoryCount].entries != NULL)
	{
		++testCategoryCount;
	}

	testCategoryIndex = b2Clamp(testCategoryIndex, 0, testCategoryCount-1);
	testCategorySelection = testCategoryIndex;
	...
	//glui->add_statictext("Tests");//only removed to keep everything within 640x480
	GLUI_Listbox* testCategoryList =
		glui->add_listbox("Category", &testCategorySelection);
	testList =
	...
	int32 id = 0;//was 'testCount', changed to avoid same name in scope
	TestEntry* e = entryCategory->entries;
	while (e->createFcn)
	{
		testList->add_item(id, e->name);
		++id;
		++e;
	}

	id = 0;
	TestEntryCategory* cat = g_testEntryCategories;
	while (cat->entries)
	{
		testCategoryList->add_item(id, cat->name);
		++id;
		++cat;
	}
